...
public class Pokemon implements Entry {
  ...
  public Pokemon(String name, Type primaryType, Type secondaryType, int[] ev, int[] iv, int baseHealth, int baseAttack,
  ...
  public int getHealth(int level) {
    return (int) Math
        .floor((((2 * baseHealth + individualValues[0] + (effortValues[0] / 4)) * level) / 100) + level + 10);
  }
  ...
  public int getStat(Statistic stat, int level) {
    ...
    if (stat == Statistic.HEALTH) {
      ...
      return (int) Math
          .floor(((((2 * baseHealth + individualValues[0] + (effortValues[0] / 4)) * level) / 100) + 5) * nature);
    } else {
      return 0;
    }
  }
